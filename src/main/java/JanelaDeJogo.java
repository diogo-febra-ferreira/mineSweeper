...
*/
    ...
    public void btnCampoMinadoActionPerformed(ActionEvent e) {
        ...
        if (campoMinado.isJogoTerminado()) {
            if (campoMinado.isJogoDerrotado()){
                JOptionPane.showMessageDialog(null, "Oh, you blew up a mine...",
                        "You lost!", JOptionPane.INFORMATION_MESSAGE);
            } else {
                JOptionPane.showMessageDialog(null, "Congratulations! You found every mine in " +
                                (campoMinado.getDuracaoJogo() / 1000) + " seconds",
                        "Victory", JOptionPane.INFORMATION_MESSAGE);

                boolean novoRecorde = campoMinado.getDuracaoJogo() < recordes.getTempoDeJogo();
                if (novoRecorde) {
                    String nome = JOptionPane.showInputDialog("Write your name...");
    ...
    MouseListener mouseListener=new MouseListener() {
        ...
        public void mousePressed(MouseEvent e) {
            ...
            var botao = (BotaoCampoMinado) e.getSource();
            //this was inverted in the template
            var y = botao.getColuna();
            var x = botao.getLinha();
            marcarQuadricula(x, y, campoMinado);
        }
